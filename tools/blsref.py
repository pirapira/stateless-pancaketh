...
def gen_consts(out):
    ...
    elements are in Montgomery form (x * 2^384 mod p) unless the name says
    _CAN (canonical) or it is an exponent/scalar (plain little-endian limbs)."""
    from py_ecc.optimized_bls12_381 import constants as kc
    lines = []
    off = [0]
    defs = []
    stores = []
    def put_words(name, words):
        defs.append(f"#define {name} (bls_c + {off[0]})")
        for i, w in enumerate(words):
            stores.append(f"  st bls_c + {off[0] + 8 * i}, {sdec(w)};")
        off[0] += 8 * len(words)
    def put_fp(name, x): put_words(name, limbs(mont(x % P)))
    def put_fp_can(name, x): put_words(name, limbs(x % P))
    def put_fp2(name, c): put_words(name, limbs(mont(c[0] % P)) + limbs(mont(c[1] % P)))
    ...
    for i, poly in enumerate(kc.ISO_11_MAP_COEFFICIENTS):
        ...
        put_words(f"C_ISO11_K{i}", sum((limbs(mont(int(c))) for c in poly), []))
    # SWU / isogeny (G2)
    put_fp2("C_ISO_3_A", fq2(kc.ISO_3_A)); put_fp2("C_ISO_3_B", fq2(kc.ISO_3_B)); put_fp2("C_ISO_3_Z", fq2(kc.ISO_3_Z))
    put_words("C_ETAS", sum((limbs(mont(fq2(e)[0])) + limbs(mont(fq2(e)[1])) for e in kc.ETAS), []))
    put_words("C_ROOTS8", sum((limbs(mont(fq2(e)[0])) + limbs(mont(fq2(e)[1])) for e in kc.POSITIVE_EIGHTH_ROOTS_OF_UNITY), []))
    for i, poly in enumerate(kc.ISO_3_MAP_COEFFICIENTS):
        defs.append(f"#define C_ISO3_N{i} {len(poly)}")
        put_words(f"C_ISO3_K{i}", sum((limbs(mont(fq2(c)[0])) + limbs(mont(fq2(c)[1])) for c in poly), []))
    size = off[0]
    with open(out, "w") as fh:
        fh.write("/* lib/bls12381_consts.pnk -- GENERATED by tools/blsref.py --consts; do not edit.\n"
                 ...
                 "   Montgomery form (x * 2^384 mod p) except the _CAN ones and the\n"
                 "   exponents / scalars, which are plain little-endian 64-bit limbs.\n"

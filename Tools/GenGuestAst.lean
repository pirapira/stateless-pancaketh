...
Parses the cpp-expanded guest with flapjack's Pancake parser and prints the
resulting declarations as Lean source: one definition per top-level
declaration and `guestAst`, the list of them all. `Guest.AstParse` then proves
that parsing `Guest/guest.pp.pnk` gives exactly `guestAst`, so everything else
can be stated about the committed AST without re-parsing.
-/
...
def header : String :=
"-- Generated by `lake exe gen-guest-ast Guest/guest.pp.pnk`; do not edit by hand.
...
def render (declarations : List (Decl (BitVec 64))) : String :=
  ...
  header ++ "\n".intercalate definitions ++
    ...
    ",\n    ".intercalate (declarations.map declName) ++ " ]\n\nend Guest\n"

end GenGuestAst

def main (args : List String) : IO UInt32 := do
  let some (path : String) := args[0]? | do
    IO.eprintln "usage: gen-guest-ast <guest.pp.pnk>"
    return 1
  ...
  | .error errors =>
      ...
      return 1
  | .ok declarations =>
      IO.print (GenGuestAst.render declarations)
      return 0
